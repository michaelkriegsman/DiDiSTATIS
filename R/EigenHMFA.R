...
#'
#' @param HMFA_collapsed All output related to individuals, Groups, and Grand Consensus
...
EigenHMFA <- function(HMFA_collapsed, DESIGN = NULL, n2k=NULL){
  ...
  res_Grand$ProjGroup$F <- array(NA, dim=c(dim(res_Grand$eig$F),D))
  dimnames(res_Grand$ProjGroup$F) <- list(rownames(res_Grand$eig$F), colnames(res_Grand$eig$F), colnames(DESIGN_tables))
  ...
  res_Grand$ProjCP$F <- array(NA, dim=c(dim(res_Grand$eig$F),CD))
  dimnames(res_Grand$ProjCP$F) <- list(rownames(res_Grand$eig$F), colnames(res_Grand$eig$F), rownames(DESIGN_tables))

#Print_DiDiSTATIS_Ex1.2_AV_to_pptx.R
#for instructions, see: http://www.sthda.com/english/wiki/create-and-format-powerpoint-documents-from-r-software

#AV differs from Composers and Musicians in its Flip_axis2, and it adds a slide for each fixed confusion.
Flip_axis1 <- FALSE
Flip_axis2 <- TRUE
...
doc <- addTitle(doc, paste0("DiDiSTATIS Results: Ex1.2 AV \n MFA2 = ", res_DiDiSTATIS$input$MFA2_Flag))
doc <- addDate(doc)
...
plot_F.B.. <- function(){ Plot_DiDiSTATIS_F.B..(res_DiDiSTATIS, axes = c(3,2),
                                                Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
doc <- addPlot(doc, plot_F.B.., vector.graphic = TRUE)
...
plot_FAB.. <- function(){ Plot_DiDiSTATIS_FAB..(res_DiDiSTATIS, axes = c(3,2),
                                                Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
doc <- addPlot(doc, plot_FAB.., vector.graphic = TRUE)
...
# Slide 3.5 : Add plot: FAB.. & Confusion_Grand_Fixed
#+++++++++++++++++++++++
doc <- addSlide(doc, "Two Content")
doc <- addTitle(doc, "F_Disc^.. & Confusion_Grand_Fixed")
#
...
#
plot_Confusion_Fixed_Grand <- function(){ PlotConfusion(res_DiDiSTATIS$res_BaryGrand$Prediction_Fixed_Rows$Confusion_Rows_norm, scale_max_100 = F, dev_new = F) }
doc <- addPlot(doc, plot_Confusion_Fixed_Grand, vector.graphic = TRUE)
doc <- addFooter(doc, paste0("r2_Categories^.. = ", round(res_DiDiSTATIS$res_BaryGrand$EffectSize$r2_Categories.., 2)))
...
plot_F.B.D <- function(){ Plot_DiDiSTATIS_F.B.D(res_DiDiSTATIS, axes = c(3,2),
                                                Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
doc <- addPlot(doc, plot_F.B.D, vector.graphic = TRUE)
...
for(d in 1:res_DiDiSTATIS$input$DESIGN_tables$D){
  ...
  Plot_FAB.d <- function(){ Plot_DiDiSTATIS_FaB.d(res_DiDiSTATIS, priority = "d", connect = d, quiet_B.. = F, axes = c(1,2),
                                                  Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
  ...
  # Slides 8.5-10.5 : Add plot: Confusion_Fixed_Group_d & FABd
  #+++++++++++++++++++++++
  doc <- addSlide(doc, "Two Content")
  doc <- addTitle(doc, paste0("Group: ", res_DiDiSTATIS$input$DESIGN_tables$labels[d], "\n Confusion_Fixed & F_AB^.d"))
  Plot_FAB.d <- function(){ Plot_DiDiSTATIS_FaB.d(res_DiDiSTATIS, priority = "d", connect = d, quiet_B.. = F, axes = c(1,2),
                                                  Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
  doc <- addPlot(doc, Plot_FAB.d, vector.graphic = TRUE)
  plot_Confusion_Fixed_Group_d <- function(){ PlotConfusion(res_DiDiSTATIS$res_BaryGrand$Prediction_Fixed_Rows$Confusion_norm.d[,,d], scale_max_100 = F, dev_new = F) }
  doc <- addPlot(doc, plot_Confusion_Fixed_Group_d, vector.graphic = TRUE)
  doc <- addFooter(doc, paste0("SS_B^.d = ",             round(res_DiDiSTATIS$res_BaryGrand$EffectSize$SS_.b.D[d], 2),
                               "\n SS_A(B)^.d = ",       round(res_DiDiSTATIS$res_BaryGrand$EffectSize$SS_aINb.D[d], 2),
                               "\n r2_Categories^.d = ", round(res_DiDiSTATIS$res_BaryGrand$EffectSize$r2_Categories.D[d], 2)))
...
if("Boot_Tables" %in% names(res_DiDiSTATIS)){
  ...
  plot_Boot_CIs <- function(){ Plot_DiDiSTATIS_Boot_centered_CIs_BD(res_DiDiSTATIS, axes = c(1,2), Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
  doc <- addPlot(doc, plot_Boot_CIs, vector.graphic = TRUE)
  plot_Boot_CIs <- function(){ Plot_DiDiSTATIS_Boot_centered_CIs_BD(res_DiDiSTATIS, axes = c(3,2), Flip_axis1 = Flip_axis1, Flip_axis2 = Flip_axis2, dev.new = F) }
  doc <- addPlot(doc, plot_Boot_CIs, vector.graphic = TRUE)
}
